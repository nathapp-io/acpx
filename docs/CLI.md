...
| `--model <id>`                           | Set agent model                                | Overrides `agents.<name>.model` from config. Claude-compatible adapters may consume session creation metadata; other agents must advertise a model config option or legacy `models` metadata. |
